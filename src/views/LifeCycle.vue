...
<script setup lang="ts">// <script setup> 单文件组件模式： Single-File Component，简称 SFC

import { useRouter } from 'vue-router';
import {
    onMounted, onUnmounted, onBeforeMount, onBeforeUpdate, onUpdated,
    onBeforeUnmount, onErrorCaptured, onRenderTriggered, onRenderTracked, onActivated
} from 'vue';
...
function goOptionPage() {

    router.push({ name: 'LifeCycleOptionAPI' });
...
onBeforeMount(() => {
    console.log('onBeforeMount 组件即将被挂载');
})
...
onBeforeUnmount(() => {
    ...
    console.log('onBeforeUnmount 组建即将卸载')

})

onUnmounted(() => {
    console.log('onUnmounted 组建卸载')
...
<template>
    <div class="lifeCycle">
        ...
        <h2>Vue3 LifeCycle</h2>

        <br />

        <button @click="goOptionPage">Go Option API Mode</button>
